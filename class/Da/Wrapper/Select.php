...
class Da_Wrapper_Select extends Da_Wrapper_Abstract
{
	...
	public function groupby()
	{
		...
		if ($argc > 0) {
			if (is_array($argv[0])) {
				$this->_groups = $argv[0];
				return $this;
			} elseif ($argv[0] === FALSE || empty($argv[0])) { // cancel group
				$this->_groups = NULL;
				return $this;
			}
			$this->_groups = $argv;
	...
	private function getDoSth($count = false)
	{
		...
		if(is_array($input_parms) && count($input_parms) > 0) {
			...
			if(!$ret) {
				Log::warning(__CLASS__ . '->'. __FUNCTION__ .' execute: '.print_r($sth->errorInfo(), true) . ' sql: ' . $sql);
			}
		} else {
			$sth = $dbh->query($sql);
			if(!$sth){
				Log::warning(__CLASS__ . '->'. __FUNCTION__ .' query: '.print_r($dbh->errorInfo(), true) . ' sql: ' . $sql);
			}
		}
		if(!$sth) {
			Log::warning(__CLASS__ . '->'. __FUNCTION__ . ' table : ' . $this->_table . ' : ' . print_r($dbh->errorInfo(),true) . ' sql: ' . $sql);
		}
		return $sth;
	}
