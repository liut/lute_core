...
class Cache_Memcache extends Cache // implements ArrayAccess
{
	...
	{
		//$this->_group = defined('CACHE_MEMCACHE_GROUP') ? preg_replace('/[^a-z0-9_:]+/i', '', CACHE_MEMCACHE_GROUP) : '';
	}
	...
	public function addServers(array $servers)
	{
		...
		if (is_object($this->_memcache)) {
			foreach($servers as $server) {
				if (call_user_func_array([$this->_memcache, 'addServer'], $server)) {
					$this->_connected = true;
					if($this->_debug)
					{
						$message = sprintf("addServer %s:%d OK!", $server[0], isset($server[1]) ? $server[1] : 11211);
						$this->log($message);
					}
				}
			}
