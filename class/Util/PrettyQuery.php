...
class Util_PrettyQuery
{
	...
	public function init($uri = null)
	{
		...
		$pos = $this->prefix ? strpos($uri, $this->prefix) : 0;
		if($pos === FALSE) {
			$this->vals = array_fill_keys($this->keys, '');
			return $this;
		}
		$str = substr($uri, $pos + strlen($this->prefix));
		if ( $this->suffix &&  ($pos = strrpos($str, $this->suffix)) !== FALSE) {
		...
		$this->vals = array_combine($this->keys, array_map('rawurldecode', $u_vals));
	...
	public function url($name = null, $value = '', $name2 = null, $value2 = '')
	{
		...
		return $this->prefix . rtrim($this->seed($arr), $this->glue) . $this->suffix;
	}
	/**
	 * 返回可用于标识本次实例值的字串种子
	 * 
	...
	public function seed($vals = null)
	{
		...
		return implode($this->glue, array_map('rawurlencode', $vals));
	}
