...
class Util_Recency
{
	...
	public function save()
	{
		if($this->_changed) {
			...
			$now = Request::current()->TIME;
			if ($domain) {
				return setcookie($this->_cookie_name, implode($this->_glue, $this->_ids), $now + $this->_cookie_life, '/', $domain);
			}
			return setcookie($this->_cookie_name, implode($this->_glue, $this->_ids), $now + $this->_cookie_life, '/');
		}
	}
