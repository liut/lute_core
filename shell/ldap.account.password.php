...
include_once __DIR__ . '/../tests/init.php';
include_once __DIR__ . '/../function/ldap.func.php';
...
$cfg = Loader::config('admin_ldap');
if (!is_array($cfg) || empty($cfg)) {
	echo 'config admin_ldap.* not found', PHP_EOL;
	return;
}
$cfg = reset($cfg);

// connect to ldap server
$ldapconn = ldap_connect($cfg['ldap_host'])
...
if ($ldapconn) {
	...
	$username = $argv[1];
	// using ldap bind
	$ldaprdn  = sprintf('uid=%s,%s', $cfg['ldap_base_dn'], $username);     // ldap rdn or dn
	$ldapbind = null;
